...
namespace Saandy
{
	...
	public static class Math2d
	{
		...
		public static bool PointIsWithinMesh( Vector3 point, ModelRenderer renderer )
		{
			...
			{
				uint a = indicies[i * 3 + 0];
				uint b = indicies[i * 3 + 1];
				uint c = indicies[i * 3 + 2];
				Triangle tri = new Triangle( vertices[a], vertices[b], vertices[c] );

				BBox aabb = tri.GetAABB();
				//Gizmo.Draw.LineBBox( in aabb );
			...
			if ( iTriCount % 2 == 0 )
			{
				// Intersected an even number of triangles
				return false;
			}

			// Intersected an odd number of triangles, which means we're inside the mesh!
			return true;
